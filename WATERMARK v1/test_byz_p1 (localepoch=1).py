...
##### add backdoor dataset and train backdoored global model
        print("# --------------------------read dataset+ add trigger: %s --------------------------" % args.dataset)
        train_data_dataset_addtrigger, test_data_dataset_addtrigger_ori, test_data_dataset_addtrigger_tri = load_data_dataset_addtrigger(args.dataset,args.trigger_label, args.poisoned_portion, mx.cpu())
        #testtesttesttesttesttesttest
        print("# ---test data sample structu --------------------------",len(train_data_dataset_addtrigger))
        #
        print ("# --------------------------construct poisoned dataset--------------------------")
        p_train_data_loader, p_test_data_ori_loader, p_test_data_tri_loader = create_backdoor_data_loader(args.dataset, train_data_dataset_addtrigger, test_data_dataset_addtrigger_ori, test_data_dataset_addtrigger_tri, args.trigger_label, args.poisoned_portion, mx.cpu())
...
################ LOAD PARAMETERS from trained watermark server model
           # new_net = get_net(args.net, num_outputs)
            net.load_parameters('net_watermark_server_250GE_BrownCube.params',ctx = ctx)
...
#            logging.info("################Communication round : {}".format(e))
     ...
     #       train_loss_avg = sum(train_locals_loss)/len(train_locals_loss)
         ...
         #####need to load_parameters from global model for every worker##################
                ...
                nd_aggregation1.watermarkfl(param_list_after, net, lr, local_weighted_score)
            net.save_parameters('guochengRecord')
            del grad_list
            grad_list = []
