...
def get_byz(byz_type):
    ...
    elif byz_type == 'mean_attack':
        return byzantine1.full_mean_attack
    elif byz_type == 'scaling_attack':
        return byzantine1.scaling_attack
    else:
        raise NotImplementedError
...
#            logging.info("################Communication round : {}".format(e))
     ...
     #           train_loss_avg = sum(train_locals_loss)/len(train_locals_loss)
                ...
                if e <= 2:
                    ...
                    nd_aggregation1.watermarkfl(param_list_after, net, 0.0001, local_weighted_score, trust_value)
                    net.save_parameters('guochengRecord')
                else:
...
###########reload global model for real training#######################
            net.load_parameters('net_watermark_server_250GE_BrownCubeCIFAR100.params',ctx = ctx)
############trust_value GOT! net paras GOT!#####################################################
        # begin real training
