...
def trim_attack(v, net, lr, f):
    ...
    #print('maximum_dim,direction,directed_dim',maximum_dim,direction,directed_dim)
...
def dir_full_krum_lambda(v, net, lr, f):
    ...
    return v
def mean_attack(v, net, lr, f):
    for i in range(f):
        v[i] = -v[i]
    return v

def full_mean_attack(v, net, lr, f):
    if f == len(v):
        for i in range(f):
            v[i] = -v[i]
        return v
